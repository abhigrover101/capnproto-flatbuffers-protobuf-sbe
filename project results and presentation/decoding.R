...
print(brks)
png(file="decoding-cdf.png", height=800, width=800, pointsize=12)
xlabel <- expression(paste("Time to decode (ns)"))
p <- ggplot(d, aes(x=Time,y=Percentile,colour=Serializers))

p + geom_line(size=2) + 
	theme(panel.grid.minor = element_blank(),text = element_text(size=20)) +
	scale_x_log10(xlabel,breaks = c(brks,1500,5000,10000),limits=c(xmin, xmax999th)) +
